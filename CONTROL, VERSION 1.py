...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul 23 10:19:28 2026

@author: maggie
"""

#Model 1 --> iteration 6 
#CONTROL CODE, NO COMPARTMENTS FORMING
#THEREFORE:
    #- no growth 
    #- no occupancy 
    #- baseline vsicosity 
    #- baseline diffusion 
    #- local flow is the same as baseline flow
    #- transport coeffiecnt = 1 (see math below)
    #- phase lag = 1 (see math below)
...
#WORK FLOW:
    ...
    #Highway Fraction --> caluclated, from curr_radii / MAX radii (ImageJ)
...
def dyn_visc(occupationalfraction):
    ...
    dynamicvisco = cyto_visc 
    return dynamicvisco

def local_flow_calc(vis, ugrowth):#boundary condition here!

    ulocal =  vel_baseline #simple for now but can change 
    return ulocal

def eff_dif(visco):
    
    Deff = baseline_diffusion 
    return Deff

def transport_coe_calc(localvelocity, effectivediffusion):
    
    #this varies from mthe background processes that are surely happening
    #and affecting the tranport as a whole.
    background_transport = 1.0 + 0.15*np.sin(2*np.pi*i/(6*T)) #randomizerish, so it's not consistently pi
    TransCoe = background_transport
    return TransCoe
...
for i in range(0, t, dt):
    ...
    #local Flow stuff
    ulocal_carrier = local_flow_calc(dynamic_visocity_carrier, growth_rate_carrier)
...
plt.plot(minute, highway_openness_fraction, color = 'blue', label = 'highway openness factor')                                
plt.legend()  
for x in range(60,1201,60):
    plt.axvline(x, linestyle='--', alpha=0.3, color='black')                             
plt.show()
...
#I think the max lag is about pi, not 2pi --> this assumption can be changed, it just helps set scale for experimental data
y_labels = ['0','π/2','π']
plt.xlabel("Time (minutes)")
plt.ylabel("Relative Phase Lag")
