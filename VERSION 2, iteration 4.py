...
max_force_releasable = 0.3 #Newtons, adjust this to match the largest force later
baseline_CDK1_wave_speed = 10 #microns / second
baseline_diffusion = 1.0 
drag_force = 0.05 #edit later depending on order of mag
tau = 1.0
alpha = 2175 #might make this vairable, not sure yet. 
beta = 1
theta = 0.65 # will tweak this later 
scaling_const = 3e3 # edit this later, may not need to be anything more than 1
scaling_std = 1805 #see excel
lag_mins_carrier = 8 #starting lag 
...
plt.scatter(x, cycle_periods, color="blue")
...
plt.plot(cycle_end_times, cycle_periods , label = 'Period Length 2', color = 'm')  
plt.legend()  
plt.show() 
...
plt.plot(x, slope*x + intercept, color="green")
equation = f"y = {slope:.2f}x + {intercept:.2f}\n$R^2$ = {r_value**2:.2f}"
plt.scatter(x, cycle_lags, marker='o')
...
plt.xlabel("Cycle Number")
plt.ylabel("Phase Lag (min)")
plt.title("Predicted Lag per Cycle")
plt.show()
